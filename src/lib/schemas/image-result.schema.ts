...
export const imageResultSchema = z.object({
  ...
  contentImages: z.array(z.string().min(1)).length(5),
  thumbnailText: z.string().min(1),
  altTexts: z.array(z.string().min(1)).length(6),
  fileNames: z.array(z.string().min(1)).length(6),
  coverImageUrl: z.string().min(1),
  contentImageUrls: z.array(z.string().min(1)).length(5),
...
export function imageResultToBlogAssets(result: ImageResult) {
  return {
    ...
    altTags: {
      ...
      body: result.altTexts.slice(1, 6),
    },
    imageFilenames: {
      representative: result.fileNames[0],
      body: result.fileNames.slice(1, 6),
    },
